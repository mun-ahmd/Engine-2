...
public:
  CamHandler(GLFWwindow *window) {
    this->cam = Camera(glm::vec3(0.0f, 0.0f, 3.0f), glm::vec3(0.0f, 1.0f, 0.0f),
                       glm::vec3(0, 1, 0));
  ...
  void lookAround() {
    double xPos, yPos;
    double lastX = this->mouseLastX;
    double lastY = this->mouseLastY;
    glfwGetCursorPos(this->currentWindow, &xPos, &yPos);

    // if first mouse input....
    // this stops massive offset when starting due to large diff b/w last pos
    // and curr pos of mouse
    ...
    lastX = xPos;
    lastY = yPos;
    this->mouseLastX = lastX;
    this->mouseLastY = lastY;

    this->cam.mouseLookProcess(xOffset, yOffset, true);
  }
  glm::mat4 getView() { return this->cam.getViewMatrix(); }
  glm::vec3 get_pos() { return this->cam.position; }
};
...
private:
  std::unordered_map<unsigned int, unsigned int>
      id_to_index; // in most cases id would be vertex array id
...
public:
  ...
  bool deallocate(unsigned int id) {
    ...
    buff.copy_self(size_used - offsets_ids[deallocate_offset_index + 1].first,
                   offsets_ids[deallocate_offset_index].first,
                   offsets_ids[deallocate_offset_index + 1].first);
    // todo ^ might need to change it to make a new intermediate buffer
    size_t removed_size = offsets_ids[deallocate_offset_index + 1].first -
                          offsets_ids[deallocate_offset_index].first;
    size_used -= removed_size;
    offsets_ids.erase(std::next(offsets_ids.begin(), deallocate_offset_index));
    id_to_index.erase(id);

    for (auto ptr = std::next(offsets_ids.begin(), deallocate_offset_index);
         ptr < offsets_ids.end(); ++ptr) {
...
private:
  ...
  // mesh_index is the same as static mesh id, because it is assured that it is
  // unique despite deletion of meshes
  ...
  ComputeShader culling_compute =
      ComputeShader("src/Graphics/Shaders/CullClusters.compute");

  inline size_t add_mesh_store_info(size_t vertices_data_size,
    ...
    if (mesh_buffers_info.empty()) {
      ...
      info.indices_offset = mesh_buffers_info.back().indices_offset +
                            mesh_buffers_info.back().indices_size;
      info.vertices_offset = mesh_buffers_info.back().vertices_offset +
                             mesh_buffers_info.back().vertices_size;
      size_t num_clusters_prev = mesh_buffers_info.back().indices_size /
                                 (3 * num_triangles_per_cluster);
      info.clusters_offset =
          mesh_buffers_info.back().clusters_offset +
          num_clusters_prev * sizeof(ComputeShaderDataStructures::ClusterInfo);
    }
    ...
    this->mesh_information.modify(
        &mesh_inf, sizeof(ComputeShaderDataStructures::MeshInformation),
...
public:
  ...
  MultiStaticMesh(const size_t max_index_buff_size,
                  const size_t max_vertex_buff_size) {

    this->indices = Buffer(max_index_buff_size, NULL);
    this->vertices = Buffer(max_vertex_buff_size, NULL);

    this->VAO =
        VertexArray(this->vertices, this->indices, this->get_vertex_attribs());
    ...
    this->mesh_information = Buffer(
        sizeof(ComputeShaderDataStructures::MeshInformation) * max_num_meshes,
        NULL);
    this->cluster_information = Buffer(
        sizeof(ComputeShaderDataStructures::ClusterInfo) * max_num_clusters,
        NULL);
    ...
    this->indirect_draw_info =
        Buffer(sizeof(ComputeShaderDataStructures::IndirectDrawInfo) *
                   max_num_clusters,
               NULL);
  ...
  std::optional<uint32_t> add_mesh(const Vertex3 *vertex_data,
                                   size_t num_vertices,
                                   const unsigned int *indices_data,
                                   size_t num_indices) {
    size_t num_triangles_in_mesh = num_indices / 3;
    unsigned int num_add_indices =
        (num_triangles_per_cluster -
         (num_triangles_in_mesh % num_triangles_per_cluster)) *
        3;

    size_t final_num_indices = num_indices + num_add_indices;
    size_t final_num_vertices = num_vertices + (num_add_indices != 0);

    size_t mesh_buffer_info_index =
        add_mesh_store_info(final_num_vertices * sizeof(Vertex3),
                            final_num_indices * sizeof(unsigned int));
    auto mesh_store_info = mesh_buffers_info[mesh_buffer_info_index];

    this->vertices.modify(vertex_data, num_vertices * sizeof(Vertex3),
                          mesh_store_info.vertices_offset);
    this->indices.modify(indices_data, num_indices * sizeof(unsigned int),
                         mesh_store_info.indices_offset);

    if (num_add_indices != 0) {
      this->vertices.modify(&NaN_Vertex, sizeof(Vertex3),
                            mesh_store_info.vertices_offset +
                                num_vertices * sizeof(Vertex3));

      std::vector<unsigned int> extra_indices(num_add_indices, num_vertices);
      this->indices.modify(
          extra_indices.data(), num_add_indices * sizeof(unsigned int),
    ...
    std::vector<ComputeShaderDataStructures::ClusterInfo> cluster_inf(
        final_num_indices / (3 * num_triangles_per_cluster),
        same_inf_each_cluster);

    this->cluster_information.modify(
        cluster_inf.data(),
        cluster_inf.size() * sizeof(ComputeShaderDataStructures::ClusterInfo),
        mesh_store_info.clusters_offset);

    this->num_clusters += cluster_inf.size();
    this->num_meshes += 1;

    // mesh_index is the mesh id
    this->id_to_buffer_info[mesh_store_info.mesh_index] =
        mesh_buffer_info_index;
  ...
  void create_indirect_draw_buffer() {
    ...
    struct indirect_draw_struct {
      ...
      unsigned int firstIndex; // number of indices before the first one of this
                               // cluster regardless of which mesh it belongs to
      unsigned int baseVertex; // number of vertices before the first one of
                               // cluster's mesh
    ...
    for (const auto &mesh_buf_info : this->mesh_buffers_info) {
      unsigned int num_clusters_in_curr =
          mesh_buf_info.indices_size /
          (sizeof(unsigned int) * 3 * num_triangles_per_cluster);
    ...
    this->indirect_draw_info.modify(
        buff_data.data(), buff_data.size() * sizeof(indirect_draw_struct), 0);
  }

  void set_instance(uint32_t static_mesh_id, glm::mat4 transform,
                    uint32_t material_id) {
    // todo
    auto req_mesh_index =
        this->mesh_buffers_info[this->id_to_buffer_info[static_mesh_id]]
            .mesh_index;
    this->transformations.modify(glm::value_ptr(transform), sizeof(glm::mat4),
                                 sizeof(glm::mat4) * req_mesh_index);
    this->material_ids.modify(&material_id, sizeof(uint32_t),
                              sizeof(uint32_t) * req_mesh_index);
  }

  void draw_alone(uint32_t static_mesh_id) const {
    // this is only supposed to handle the draw call nothing else including
    // transformation
    const auto &mesh_buff_info = this->mesh_buffers_info[static_mesh_id];
    VAO.draw(mesh_buff_info.indices_size / sizeof(unsigned int),
             mesh_buff_info.indices_offset / sizeof(unsigned int));
  }

  void bind_vao() const { VAO.bind(); }
  ...
  const Buffer &get_material_ids_buff() const { return this->material_ids; }
...
private:
        ...
        ComputeShader culling_compute =
ComputeShader("src\\Graphics\\CullClustersCompute.glsl");
        ...
        std::vector<VertexAttribData> get_vertex_attribs()
        {
                ...
                //attribs[3].offset = 0;
                //attribs[3].stride = sizeof(unsigned int);	//todo this wont
work; google if you dont understand why
        ...
        void set_transform(unsigned int index)
        {
                ...
                transform_matrices_buffer.modify(&zero_transform,
sizeof(glm::mat4), index * sizeof(glm::mat4));
        }
...
public:
        ...
        MultiStaticMesh_OLD(unsigned int num_triangles_per_cluster, unsigned
char indices_type_size, unsigned short per_vertex_size, size_t
max_size_vertex_buff, size_t max_size_index_buff) :
                num_triangles_per_cluster(num_triangles_per_cluster),
indices_type_size(indices_type_size), per_vertex_size(per_vertex_size)
        {
                vertices_multi = BufferMulti(max_size_vertex_buff);
                indices_multi = BufferMulti(max_size_index_buff);
                VAO = VertexArray(vertices_multi.get_buffer(),
indices_multi.get_buffer(), get_vertex_attribs());
                //glVertexAttribDivisor(3, 1);
                indirect_draw_buffer = Buffer(20 * (max_size_index_buff / (3 *
128)) * 8, NULL); mesh_information_buffer = Buffer(MAX_NUM_MESHES * 8, NULL);
                cluster_information_buffer = BufferMulti((max_size_index_buff /
(3 * 128)) * 8);

                transform_matrices_buffer = Buffer(sizeof(glm::mat4) * 1000,
NULL, GL_SHADER_STORAGE_BUFFER, 0, GL_STREAM_DRAW); material_ids_buffer =
Buffer(sizeof(unsigned int) * 1000, NULL, GL_SHADER_STORAGE_BUFFER, 0,
GL_STREAM_DRAW);

                culling_compute.add_ssbo_block("ClustersInfo");
                culling_compute.add_ssbo_block("MeshInfo");
                cluster_information_buffer.get_buffer().bind_base(GL_SHADER_STORAGE_BUFFER,
4); mesh_information_buffer.bind_base(GL_SHADER_STORAGE_BUFFER, 5);
                indirect_draw_buffer.bind_base(GL_SHADER_STORAGE_BUFFER, 3);
        }
        void multi_draw() const
        {
                //views_buff.bind_base(GL_SHADER_STORAGE_BUFFER, 6); //avoid
putting this here VAO.multi_draw_indirect(num_clusters, 0, 0);
        }
        ...
        void create_indirect_draw_buffer()
        {
                //prepares data cpu side , no compute shader, use this for
debugging auto verts_offsets = vertices_multi.get_all_offsets(); auto
indices_offsets = indices_multi.get_all_offsets(); size_t num_indices =
indices_multi.get_used_size() / indices_type_size;
                ...
                std::vector<indirect_draw_struct> buff_data;
                buff_data.reserve(num_indices / (3 *
num_triangles_per_cluster));	//want x triangle groups, that being 3*x indices
per group
                ...
                {
                        unsigned int baseVertex = (verts_offsets[i]) /
per_vertex_size;

                        if ((indices_offsets[i + 1] - indices_offsets[i]) /
indices_type_size >= num_triangles_per_cluster * 3)
                        {
                                for (unsigned int x = 0; x < (indices_offsets[i
+ 1] - indices_offsets[i]) / indices_type_size; x += num_triangles_per_cluster *
3)
                                {
                                        indirect_draw_struct curr;
                                        curr.count = num_triangles_per_cluster *
3; curr.instanceCount = 1; curr.baseVertex = baseVertex; curr.firstIndex =
indices_offsets[i] / indices_type_size + x; std::cout << "\n" << curr.firstIndex
<< "\n"; curr.baseInstance = i; buff_data.push_back(curr);
                                }
                        }
                        else
                        {
                                indirect_draw_struct curr;
                                curr.count = (indices_offsets[i + 1] -
indices_offsets[i]) / indices_type_size; curr.instanceCount = 1; curr.baseVertex
= baseVertex; curr.firstIndex = indices_offsets[i] / indices_type_size;
                                curr.baseInstance = i;
                                buff_data.push_back(curr);
                        }
                }

                indirect_draw_buffer.new_data(buff_data.data(), buff_data.size()
* sizeof(indirect_draw_struct));
        }
        ...
        void debug_output_indirect_draw(std::string filename)
        {
                ...
                indirect_draw_buffer.access(buff_data, num_clusters * 5 *
sizeof(unsigned int), 0); file << "count, instance_count, first_index,
base_vertex, base_instance" << std::endl; for (unsigned int i = 0; i <
num_clusters * 5; i += 5)
                {
                        for (int x = 0; x < 5; ++x)
                        {
                ...
                mesh_information_buffer.access(buff_data, num_meshes *
sizeof(unsigned int) * 2, 0); for (unsigned int i = 0; i < num_meshes * 2;i +=
2)
                {
                        file << buff_data[i] << ',' << buff_data[i + 1] <<
std::endl;
                }
                ...
                cluster_information_buffer.get_buffer().access(buff_data,
sizeof(unsigned int) * num_clusters * 2, 0); for (unsigned int i = 0; i <
num_clusters * 2;i += 2)
                {
                        file << buff_data[i] << ',' << buff_data[i + 1] <<
std::endl;
        ...
        {
                transform_matrices_buffer.modify(&transform, sizeof(glm::mat4),
index * sizeof(glm::mat4));
        }
        void set_transform(MeshStatic* mesh, glm::mat4 transform)
        {
                transform_matrices_buffer.modify(glm::value_ptr(transform),
sizeof(glm::mat4), vertices_multi.get_object_index(mesh->get_vao()) *
sizeof(glm::mat4));
        }
        void set_mat_id(MeshStatic* mesh, unsigned int  mat_id)
        {
                material_ids_buffer.modify(&mat_id, sizeof(unsigned int),
vertices_multi.get_object_index(mesh->get_vao()) * sizeof(unsigned int));
        }
        ...
        inline unsigned int get_num_triangles_per_cluster() const { return
num_triangles_per_cluster; }
};
*/
